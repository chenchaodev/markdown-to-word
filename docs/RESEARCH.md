...
- **理由**:①的失败形态是「产物结构合法、字符无缺失、但观感坏了」—— 查缺字形、查字符集都找不到,只有直接看 XML 结构才暴露;③则说明断言只查「结构里有没有 `m:nary`」会漏掉 `m:e` 为空这类缺陷。
- **来源**:2026-09-26 用户在 WPS 目视发现方框后定位(空 `<m:e/>` + 被加数漏成兄弟节点);与 docx 库源码实现及其自带示例双向核对。
- **关联**:`src/core/docx/handlers/math.ts`(`munderoverToNary` 增 `operand` 参并按「只消费紧邻一个兄弟」取被加数,取不到即整式降级)、`src/core/docx/handlers/equations.ts`(`renderContainerMath` 供列表/引用块复用,与顶层同一条管线)、`src/core/docx/handlers/content.ts`(列表/引用块的 `math` 分支)、`test/segments/math-structures.test.js`(断言须查 `<m:e>` 内有被加数、且 `</m:nary><m:r>` 不得出现)。**已知遗留**:行内 `∑` 仍是 `m:sSubSup` + 被加数作兄弟(Word 原生会用 `m:nary` + `limLoc subSup`),如需对齐另立工作项。

### 2026-09-26 21:30:00 Electron 门禁入口的三条失败路径事实(与直觉相反,勿回退)
- **结论**:① **`app.quit()` 不吃 `process.exitCode`** —— 实测设 `process.exitCode = 1` 后调 `app.quit()`,真实退出码是 **0**;抛错后再走 `app.quit()` 同样是 0。**失败被判绿,比挂死更隐蔽**。要显式码必须用 `app.exit(code)`(ready 前/后都吃)。纯 Node 脚本里 `process.exitCode` 正常生效,这条只针对 Electron 主进程,别把纯 Node 脚本一起改。② **加载期抛错进程内无解**:Electron 的 ESM 主模块 loader 捕获求值错误后只打 `App threw an error during load`,**不退出**,此时入口文件里没有任何代码在跑、没有钩子能接管。更深一层:ESM 在**任何求值之前**先完成整张模块图的 linking,故「静态 import 解析不到/缺导出」发生在守卫自己被加载之前 —— 只能靠「把入口静态导入面压到最小(node 内建 + electron + 守卫),业务依赖一律动态 import」来缩小这个盲区,`M2W_ENTRY_LOAD_TIMEOUT_MS` 看门狗兜「载荷既不完成也不报错」。③ **`.then()` 回调内抛错只降级为 `UnhandledPromiseRejectionWarning`、进程照活** —— Electron 主进程不采用 Node 默认的 `unhandled-rejections=throw`;`await app.whenReady()` 写在 async 函数体内(不是模块顶层)时同样如此。与已记的「顶层 `await app.whenReady()` 挂起」是**两个不同的坑**,后者只发生在真正的模块顶层 await。
- **理由**:三条都让门禁在失败时给出**错误的信号** —— 挂死会让 CI 报「超时」而非「你的门禁失败了」,静默判绿则更糟(永远不会有人发现)。而它们都不报错、只表现为「结果不对」,不实测就一定会按直觉写错。
- **来源**:2026-09-26 实测(每条都真起 Electron 子进程验退出码,含「抛错 + `app.quit()` 仍为 0」这一对照);修复见 `test/common/entry-guard.mjs`。
- **关联**:`test/common/entry-guard.mjs`(`decideEntryExit` 纯函数判定层 + `runEntry` 壳层,五个入口共用;失败关闭:阶段非 ok、阶段 ok 却带错误、阶段不在码表一律非零)、`test/segments/entry-exit-guard.test.js`(三层断言,含真实子进程端到端)、`scripts/check-geometry.mjs`(静态导入面收到只剩 electron + 守卫)、`test/common/segment-host.mjs` 与 `test/acceptance.mjs`(段宿主与验收入口)、`test/tools/visual-check.mjs`(旧写法是「`app.quit()` + `exitCode=1`,实测退出 0」)。**已知残余**:`session-persist-feedback` 段会 `process.removeAllListeners("unhandledRejection")` 摘掉兜底监听,此后回到 warn + 挂住(真实失败仍由壳层 try/catch 覆盖)。

### 2026-09-26 21:30:00 测试框架的「选择面」与「发现面」是两个语义
- **结论**:`M2W_ONLY` 同时被两件不同的事消费 —— 「harness 这一轮跑哪些**顶层**段」与「某段内部 `runAll`/`discoverSegments` 能发现哪些段」。旧实现里 `discoverSegments(dirs)` 无条件读 `process.env.M2W_ONLY`,而段宿主把 `...process.env` 整包传给子进程,于是**顶层筛选词跨进程渗进段内**:段自测要跑的沙盒段前缀是 `segments/xxx-selftest/`,被外层 `M2W_ONLY=segments` 命中零个 → 发现面被滤空 → 拿到空清单。**滤空发生在发现层,不是执行层** —— 执行层只是忠实执行了被滤空的清单,查错方向会一路错到执行层。
- **为什么长期没暴露**:触发条件是「筛选词命中零个目标段」,而按段名前缀筛(如 `M2W_ONLY=runner-report`)恰好**不**触发(它是沙盒名前缀的子串);且 CI 全量跑不设 `M2W_ONLY` —— 所以只在本地按目录筛的隔离调试姿势下暴露。实证信号是同一段内两种用法并存:一处要「不筛选」、两处要「按词筛」,而旧 API 只能靠改环境变量表达后者、无法表达前者。
- **来源**:2026-09-26 实测(按目录筛时某段报「应执行 N 个沙盒段,实际 0」)。
- **关联**:`test/common/runner.js`(`resolveOnlySelection` 承载三态契约:未声明=读 `M2W_ONLY`、显式 `null`=不筛选、字符串=按词筛;`runSegmentIsolated` 边界把该变量从子进程 env 删除,段清单已由 `M2W_SEGMENT_FILE` 单独决定)、`test/segments/runner-report.test.js`。**已知遗留**:`test/segments/fixture-contract.test.js` 是第二处段内自用该变量 —— 手工 `delete` 环境变量再 `finally` 复原,属调用点打补丁,新契约下其行为不变且仍正确,改写只是可读性。

### 2026-09-26 20:10:00 asar 归档格式两个反直觉事实(勿回退)
- **结论**:① asar 头部的文件树是**按目录分层嵌套**的(`{"files":{"dist":{"files":{"main":{"files":{"smoke.js":{…}}}}}}}`),**拼接路径字面量(如 `dist/main/smoke.js`)在头部文本中根本不会出现** —— 用「读文件头若干 MB 再对拼接路径做子串搜索」判定条目是否存在,**必然假阴性**。本项目实测:10643 个文件的包内头部 JSON 约 2.6MB(**在 4MB 之内**),但因上述嵌套结构仍报「未找到冒烟入口」,与同一轮实际退出码 0 通过的结论**直接矛盾**。正确做法是**按路径分段在目录树里逐层下探**。② 头部定长布局为 `[u32 payload_size=4][u32 头长度][u32 头长度-4][u32 JSON 长度][JSON…]`,**JSON 起点是绝对偏移 16**。Chromium pickle 为 `[payload_size][value]`,迭代器会**跳过 payload_size**,故长度字段在**偏移 4** 而非 0 —— 按偏移 0 读会差 4 字节并解出垃圾前缀。
- **理由**:这两点各让一次判定「看起来有模有样、实则结论错误」,且都与直觉相反(头很大、路径应该能找到)。已固化于 `scripts/smoke-proc.mjs` 的 `asarContainsEntry`(按树下探,返回 `parsed` 字段区分「未收录」与「头解不开」)与 `scripts/pack-size.mjs` 的 `readAsarTree`(展平整树求和,两者**刻意不合并**:一个只答单条目在不在,一个要全树求和)。
- **来源**:实测 2026-09-26 —— `npm run dist` 全链通过的同一轮里,预检输出「打包产物收到 --smoke 必然以退出码 1 结束」而同一步骤实际 exit 0;直接 dump 真实 asar 前 32 字节得到定长布局。
- **关联**:`scripts/smoke-proc.mjs`、`scripts/pack-size.mjs`、`scripts/check-asar-manifest.mjs`(冒烟入口已升级为必备条目硬门禁)、`test/segments/install-smoke.test.js`(夹具改为构造真实 asar 二进制头,因写假 JSON 文本验证的是一个不存在的格式)。
...
- **理由**:这类失败极具误导性 —— 看断言名字会以为业务逻辑回归,实际是 stub 缺方法;而 `instanceof` 的写法在浏览器能跑、在测试里直接炸。
...
- **结论**:D-01~D-12 裁决此前只落在优化计划里,USER-GUIDE/ACCEPTANCE/BACKLOG/WPS-COMPAT 与测试注释存在旧口径。本轮把 **D-02/D-03/D-04/D-08** 写进对应文档并登记取代关系(不改写历史 CHANGELOG/ADR 原文,`[x]` 实测条目一律保留):①**D-02** 内置 `TEMPLATE_PRESETS` 携带完整交付链(typography + pageSetup + headerFooter + watermark + equationNumbering + breakBeforeH1,`src/core/settings/settings-defaults.ts:424`),套用时逐组写入并 toast 列出被覆盖分组(`src/renderer/settings/settings-bindings-preset.ts:81`);`CustomPreset` 契约**只含 typography + pageSetup**(`settings-defaults.ts:221`),故自定义预设不动页眉页脚/水印 → 界面「不入预设」徽标 = 仅指自定义预设。**实现差异留档**:删除自定义预设后的回退路径(`settings-preset-actions.ts` `deleteCustomPreset`)也只还原 typography + pageSetup,与「套用内置预设」不同链,属另开实现项,勿在文档写成完整链回退。②**D-03** 本地图片校验顺序固定为 原始 src → 词法根边界 → realpath → 规范根边界,策略单一来源 `src/core/markdown/precheck.ts` `normalizeRelativeImageSource`/`createLocalImagePathPolicy`,main 侧 `src/main/services/image-downloader.ts` 复用;拒绝面 = 绝对路径、UNC、`file://` 及一切带协议 URL、越界 `..`、realpath 越界(链接指向源目录外);**媒体类型(魔数白名单)与单文件大小上限尚未实施**,排阶段 3 资源预算——勿在文档写成已有限制。③**D-04** 现阶段不接入代码签名(不采购证书、不因缺签名阻断发布),未签名保留为明确风险;发布 SHA-256/发布说明/SmartScreen 提示/签名状态记录/产物验证由阶段 6 承接。④**D-08** 正式口径为测试段逐子进程隔离(硬超时、资源回收、case 级报告、失败 artifact),**阶段 5 待实现**;`test/common/runner.js` + `test/acceptance.mjs` 现状仍为同进程 + 看门狗(超时段记失败、继续跑、收尾 `app.exit` 硬退出释放残留资源),BACKLOG 原「看门狗悬挂段隔离 = 已明确不做」标为**已被裁决取代**并保留历史行。
- **理由**:同一事实曾有两种相反表述(内置完整链 vs 不入预设),用户文档与代码冲突会直接误导交付;口径冲突必须由裁决文档单点裁定,其余文档只做指针 + supersede 注记,不得各写一套。
- **来源**:裁决文档 `docs/OPTIMIZATION-PLAN.md` §1 决策表 + §4 阶段 0 OPT-0.3;执行台账 `docs/OPTIMIZATION-CHECKLIST.md` OPT-0.3;评审原文存档 `docs/archive/20260925-201811-全库优化评审整合与迭代计划.md`(D-02/D-03/D-04 台账行);代码事实核对 = `src/core/settings/settings-defaults.ts`、`src/renderer/settings/settings-bindings-preset.ts`、`src/renderer/settings/settings-preset-actions.ts`、`src/core/markdown/precheck.ts`、`src/main/services/image-downloader.ts`、`test/common/runner.js`、`test/acceptance.mjs`;回归断言 = `test/main/image-downloader.test.js`、`test/main/converter.test.js`。
- **关联**:写入口径落点 = `docs/USER-GUIDE.md`(模板预设/页眉页脚/水印/图片增强/预检/FAQ/已知限制)、`docs/WPS-COMPAT.md`(新增「图片取值边界(D-03)」节)、`docs/ACCEPTANCE.md`(顶部 supersede 记录表)、`docs/BACKLOG.md`(处置列新增「已被裁决取代」+ D-04/D-08 行 + 已知限制新增风险/现状行 + 晋升管道新增取代规则)、`docs/design/*`(当前口径单源)、`src/renderer/index.html`/i18n(可见文案)、`src/renderer/settings/settings-preset-actions.ts`(注释)。旧评审/归档文档中的历史表述保留，不作为当前事实。
...
- **来源**:3.11.8 首次发版 Release 与 CI 双失败(2026-09-25,TS2307 批量报错日志);本地双配置 typecheck 全绿反证 dist 存在性;E5 当时仅本地验证未跑远端 CI 即随批推送。
- **关联**:修复 = `.github/workflows` 三处步骤 build 先行(release.yml 主 job、ci.yml 主 job、ci.yml Node20 地板 job,提交 d75fe3f);发版记录见 STATUS 顶栏 3.11.8 条目。
...
- **处置**:剩余约 20 项分 A(封版期维护)/B(安全契约收口)/C(core 双源收敛)/D(结构重构)/E(测试增强)五阶段排期——**五批 24 项全部完成(2026-09-25)**,决策点 C4 拍板 ①a 最小注入、E2 拍板「仅动等待,计数全保留」均随批完成;计划收官归档(见「关联」),遗留渐进/冻结/限制三类处置随需求管道重组(同日)集中入 BACKLOG(项目 backlog)
- 来源: 4 个 explorer 子代理并行只读分析 + 主会话汇总
- 关联: 原文存档 docs/archive/20260925-004712-代码分析四路盘点.md;处置计划存档 docs/archive/20260925-130122-技术债处置计划.md;归置与遗留记录 docs/BACKLOG.md(项目 backlog)、docs/STATUS.md
- **E5 摸底与 tsc 行为实证(2026-09-25 技术债 E5)**:测试树全量标注实测 83 文件 **809 错误**(69 文件有错,TS7006 隐式 any ×200 / TS18048 ×186 / TS7005 ×135 主导),印证计划「可能引入大量报错」;两个 tsc 行为——①程序内存在 JS 语法错误(shebang 不在首行)时该轮**只报语法错误、语义诊断不输出**(首轮仅 2 条,修复后 809 条;摸底数字须在语法零错误下取)②`checkJs:false` 下 `// @ts-check` 仍按文件生效(双探针:标注→TS2305/TS2532 报出、未标注→静默),故可用「配置不放开 + 注释逐文件渐进」实现无手工排除清单的类型检查;落地 = `tsconfig.test.json` + 首批 13 个零错误文件标注,余 69 文件 809 错误为渐进增长面
...
- **G1-G9 关闭记录(2026-09-25 技术债 A4 盘点)**:G1-G8 已全部有断言覆盖,后续批次补齐时断言注释直接引用原审计行号——G1 formula.test.js(munderover 非∑回落/moText)、G2 pdf-postprocess.test.js(worker 抛错/空结果/URL 循环/checkLocalImages catch)、G3 pdf-bookmarks.test.js(旧式 Dests/decodeURIComponent catch/PDFDict 间接目标)、G4 pdf-meta.test.js(无元数据 passthrough)、G5 utils.test.js(decodeNumeric 非法码点/escapeRegExp)、G6 converter.test.js+paths.test.js(open 失败/merge pdf 分支/stat 失败)、G7 mermaid-service.test.js(png 空/catch/退出兜底)、G8 十处散点逐条命中(basic-render/merge/cross-ref/eq-numbering/settings/ui-state 等);G9 维持不补。**本清单全部关闭**,候选池 E2 同步关闭
- **覆盖率门槛落地(2026-09-25 技术债 E1)**:c8+Electron 覆盖采集实测可用(`test:coverage` 报告经 sourcemap 映射回 .ts;A4 补断言后基线 stmts 93.92 / branch 88.84 / funcs 93.15 / lines 93.92,远高于计划假设的 60% 起点,故 E1③ 无需再补断言);门槛 90/85/90/90 写在 `test:coverage` 脚本内单源,ci.yml 验收步改跑该脚本——本地与 CI 同一门禁,收紧只改一处;实证:c8 每次运行自清 tmp(无陈旧数据合并虚高)、阈值不达标 exit 1(反向 99 实测)、达标 exit 0
