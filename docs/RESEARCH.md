...
- **顺带暴露的既有脆弱点**:固定路径沙盒在并发场景没有隔离(已登记 REF-026);`M2W_ONLY` 筛选词拼错会静默报绿(已登记 REF-024)。
- **来源/验证**:REF-019 验收矩阵编排(两批数据作废后重跑);**关联**:`test/segments/runner-report.test.js`、`scripts/gate-probes/contract.mjs`、`docs/ROADMAP.md` 的 REF-024 / REF-025
