...
- **验收脚本钩子**:make-batch4-sample.mjs 新增段 append 到 `app.whenReady()` try 块(第 89 行起);docx 断言 = zipContains(部件存在)/unzipPart + 字符串匹配 OOXML 片段(如 `w:lvlText w:val="%1.%2"`);PDF 断言 = 产物 .html 字符串匹配(CSS counter、class);现有 8 段,批次 8 建议新增第 9 段
