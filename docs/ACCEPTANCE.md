...
> 口径取代:本文件历史条目中与「内置/自定义预设作用域」「本地图片拒绝面」「安装包签名」「测试段隔离」四组相关的旧表述已被现行裁决取代(预设作用域见 `docs/design/settings-ia.md`「1.5 预设的两种形态」,其余见 `docs/ADR.md` ADR-012/013/014);历史原文查 git,不再在本文件复述。
