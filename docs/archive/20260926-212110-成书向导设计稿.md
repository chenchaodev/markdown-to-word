...
> **归档注记**
>
> **本文件是历史快照，不是设计依据。** 成书向导的**实现真值**在 `src/renderer/wizard/`（`book-wizard.ts` 外壳/导航/打开关闭+付印提交、`wizard-steps.ts` 版式四步、`wizard-steps-delivery.ts` 交付三步、`wizard-fields.ts` 字段校验、`wizard-runtime.ts` 运行态单例、`wizard-state.ts` 状态 reducer）与 `docs/DEV-GUIDE.md` 代码地图；**设计规范真值**在 `docs/design/ui-guidelines.md` 与 `docs/design/settings-ia.md`。本文件全文标注「设计稿（待校订）」、文案为 DRAFT，功能已随 3.10.1 之后的版本发版，故从 `docs/design/` 撤除。
>
> **留档理由**：按用户当场指令留存副本（与 `docs/archive/20260926-211455-全库优化计划与执行台账.md` 同批处理）。正文一字未改，仅在标题后插入本注记。
>
> **来源**：原 `docs/design/book-wizard.md`，随计划项 #15 撤除，commit 见 `git log`。
