> **归档注记（2026-09-25 13:01）**：本计划已收官——A-E 五批 24 项全部完成，决策点 C4/E2 均已拍板；按文档落盘规范归档，原文自此不再改写。分流指针：分项实证与结论见 `docs/RESEARCH.md`（四路盘点条目「处置/关联」及 E1/E5 附记）；遗留与归置记录入 `docs/ROADMAP.md`（「已完成」批次压缩记录 /「测试遗留」渐进两项 /「砍」smoke 计数全量化冻结 /「已知限制」看门狗悬挂段隔离）；执行提交链见 `docs/STATUS.md` 顶部批次条目。原位置 `docs/TECH-DEBT-PLAN.md` 已撤档，索引见 `docs/README.md`。
