...
- [ ] **代码块语法高亮写 docx**(中;无依赖)——逐 token 着色;pdf 侧 hljs 已就绪可复用,docx 侧需 token→TextRun 转换器 + 颜色映射(~200 行新模块 + 测试段);2026-08-16 用户确认排期
- [ ] **i18n**(中)——界面文案抽离 + 多语言(中文为主,英文兜底);2026-08-16 用户确认排期
- [ ] **文档加密**(低-中)——docx 库原生加密;pdf 侧 pdf-lib 后处理(书签注入已有经验可复用);2026-08-16 用户确认排期
