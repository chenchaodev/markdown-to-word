...
全库优化七阶段的实现与自动断言已落地,但收尾层的文档债务尚未处置:指针断链、常驻层容量超限、候选池双源(`docs/BACKLOG.md` + `docs/ROADMAP.md`)、裁决散落在待撤除的优化计划里、阶段细节外泄进常驻层。阶段 0 把文档层收成可维护的目标形态,阶段 1 承接遗留的 GUI 目视验收。做完的判据:常驻层零断链、零超限、零阶段细节,候选与裁决各有单一真值,阶段细节只在本目录。
...
> ⚠️ **同格式异源消歧**:本表 `D-0N` 与已撤除的原优化计划 §1 中同格式的 `D-01~D-12` 是**两套号** —— 后者随 #07 撤除该文件一并升格进 `../../ADR.md`(ADR-010~ADR-015),引用时必须带出处,不得只写 `D-03`。
...
| D-02 | `docs/BACKLOG.md` 废除,候选池单源落 `docs/ROADMAP.md`「候选区」;文件删除 | 全局 `templates/docs-init/ROADMAP.md` 明写「需求唯一入口 = 候选区(不另建 BACKLOG.md)」,项目侧双源与之冲突 | ADR-009 |
...
> 逐项完成情况的真值是 `git log --oneline --grep='(#'` 与各阶段提交;本索引只保留「阶段 → 结论 → 证据」一行,**不含阶段细节**(细节查 git 与 `../../RESEARCH.md`)。
...
**原「完整问题覆盖索引」(计划 ID ↔ 覆盖的 canonical 问题 ↔ 阶段)与「每个任务的完成定义」的落点**:前者是本 campaign 计划项表(#01~#22)与已落地证据的映射,证据在 git;后者的七条完成定义由本文件硬约束②(客观可判的退出条件 + 门禁命令)与 `../../DEV-GUIDE.md`「验证基线」节承担。
...
> #03 把 `../../STATUS.md` 的日期流水按四路分流拆尽、#07 撤除原优化计划与执行台账时,下列**库/框架实测事实**在 `../../RESEARCH.md` 尚无对应条目;本批对 `../../RESEARCH.md` 的写权限仅限那一处断链,故先在此留证,由 #14 升格为正式条目(**条目不删,只补**)。
