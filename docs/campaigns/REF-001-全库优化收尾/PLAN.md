...
| #16 | 1 | 原 OPT-1.2 遗留:向导/模态/背景入口的 GUI 验收 —— **已完成 2026-09-26** | — | 真实窗口下向导与模态期间的背景入口无双触发,遮罩/Esc/关窗三条路径都结算 | 人工实测(PLAN「人工验收」节逐条核对) |
| #17 | 1 | 原 OPT-1 门禁遗留:双击/快捷键/模态/向导/关闭/保存失败的 GUI 实测 —— **已完成 2026-09-26** | — | 六条路径逐一确认无双触发与悬挂;保存失败保留编辑内容并显示失败反馈 | 人工实测(PLAN「人工验收」节逐条核对) |
| #18 | 1 | 原 OPT-4.1 遗留:真实窗口 GUI/键盘全链路验收 —— **已完成 2026-09-26** | — | dropZone 与内部控件的冒泡隔离在真实窗口成立,舞台/参数条无抖动 | 人工实测(PLAN「人工验收」节逐条核对) |
| #19 | 1 | 原 OPT-4.2 遗留:冷启动/语言切换/主题切换 GUI 实测 —— **已完成 2026-09-26** | — | 无白闪与设置跳变;菜单、动态状态、标题栏即时更新 | 人工实测(PLAN「人工验收」节逐条核对) |
| #20 | 1 | 原 OPT-4.3 遗留:预设/取消/复制反馈 GUI 实测 —— **已完成 2026-09-26** | — | 内置预设按完整交付链生效、取消为中性态、复制反馈复位 | 人工实测(PLAN「人工验收」节逐条核对) |
| #21 | 1 | 原 OPT-4.4 遗留:纯键盘/读屏与深浅主题目视验收 —— **已完成 2026-09-26** | — | 纯键盘/读屏可走完选择→预览→排序→设置→向导→预检→取消→错误恢复;深浅主题对比度达标 | 人工实测(PLAN「人工验收」节逐条核对) |
| #22 | 1 | 原 OPT-7 遗留:允许版本线内 patch/minor 的逐包裁决 —— **已完成 2026-09-26**(逐包裁决结论:全部不升,依赖零变动) | — | 每个待升依赖有「升/不升」书面结论;可自动判定的部分由 `check:contract` 覆盖,许可本身是人工裁决 | `npm ls <包>` 无 peer 告警 + `npm run typecheck && npm run lint && npm run test:smoke` |
...
| #24 | 0 | 早期 ADR 拆入 `docs/adr/`;ADR.md 只留索引 + 近期条目 + 台账 —— **已完成 `43ee341`** | ADR 容量 7,982/8,000 爆线 | ADR.md ≤6,000 且条目 ≤15;`docs/adr/` 8 个文件在位 | 字符数 + 条目数核对;指针门禁(全局配置目录 tools/check-pointers.mjs)0 错误 |
| #25 | 0 | 清 `scripts/` 与 `test/` 里硬耦合的规划编号(D-04 / OPT-5.1) —— **已完成 `fc7e90d`** | 规划编号不进交付物;改注释清不掉,须三方同步 | 脚本 / 断言 / 文档口径一致,六项门禁全绿 | `npm run typecheck` + `npm run lint` + `npm run check:signature` + `npm run check:boundary` + 两段定向 `npm test` |
| #26 | 0 | `test/**` 字符串字面量里的规划编号登记为暂缓候选 —— **已完成 `9c6e046`** | #09 只清注释,字面量属行为面未动 | ROADMAP 候选区有该行且台账已 bump | 台账人工对表 |
| #27 | 0 | 文档索引登记 `adr/` + 修正已废候选池指针 —— **已完成 `fa997d0`** | #24 产生新目录;`docs/BACKLOG.md` 已废除 | 文档登记表含 `adr/`;无已废候选池裸指针 | 指针门禁(全局配置目录 tools/check-pointers.mjs)0 错误 |
| #28 | 0 | 兼容性矩阵逐格未回填登记为已知限制 —— **已完成 `5664aa7`** | WPS 矩阵 29 格仍为 ⬜,与「整体通过」表述矛盾 | ROADMAP 已知限制段有该条 | 人工核对 |
| #29 | 1 | 回写阶段一与公式观感人工实测结果 —— **已完成 `0f6dde2`** | 用户 2026-09-26 人工实测全过(A+B+C 三组) | ACCEPTANCE 57 行全 ✅、台账 A 000-057、当前待测清空且收口规则行保留 | 字符数 ≤6,000 + 指针门禁(全局配置目录 tools/check-pointers.mjs)0 错误 |
| #30 | 0 | 规划编号禁令清单扩到 `docs/` 全部文档 + 清 RESEARCH/STATUS 违规 —— **已完成 `814d4b3` + `69459a2`** | RESEARCH.md 15 处小节标题违规;AGENTS.md 仅剩 14 字符余量 | 常驻层小节标题违规 0 处;AGENTS.md ≤2,500 且容量声明行唯一 | 正则复扫 + 指针门禁(全局配置目录 tools/check-pointers.mjs)0 错误 |

> **#13 已撤销**(`scripts/check-docs-contract.mjs` 不再建:全局门禁已有项目模式,容量契约单一来源为 `docs/README.md` 容量表 + `AGENTS.md` 容量声明行)。**#24~#30 是执行中途由裁决追加的计划项**,不在初始计划内。
...
- [x] WPS 打开 `output/artifacts/math-structures.docx`:display `∑` 不显示方框、被加数不丢失、上下限排在上下方(自动断言只锁 OOXML 结构,验不了观感) · 2026-09-26
- [x] 同一样例:列表项内 display 公式对齐到列表内容栏(不按整页宽度居中飘出列表) · 2026-09-26
- [x] 同一样例:引用块灰底连续覆盖整块(表格与代码块同样带底色,无悬空灰带) · 2026-09-26
- [x] Word/WPS 打开公式产物:行内/行间公式的实际排版效果与视觉观感 · 2026-09-26
- [x] 真实安装后用户视角:开始菜单可启动、可转换一份文档、「应用和功能」可正常卸载 · 2026-09-26
