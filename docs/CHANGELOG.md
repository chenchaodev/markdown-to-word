...
## [3.10.1] - 2026-08-30
- **修复版本:合并 3.9/3.10 两次未成功发布的全部功能与实测修复(3.9/3.10 发版失败,内容并入本版;版本号三统一 package.json=tag v3.10.1=CHANGELOG [3.10.1];typecheck/lint/build 全绿,GUI 实测通过 2026-08-29)**:
  - **剪贴板直转**:不必先把 Markdown 另存为文件——主界面空态「选择文件」下方新增「粘贴 Markdown 转换」,直接读剪贴板文本写临时文件走转换;若剪贴板是文件路径则复用拖放入队;空或非文本给出提示。
  - **成书向导**:七步引导式串联 模板预设 / 页眉页脚 / 水印 / 合并 / 模板导入 + 封面页,向导内所有设置实时保存;封面 docx 走封面节、pdf 走 HTML 模板;全程无需打开 35 控件设置抽屉即可产出复杂成书。
  - **预设覆盖扩面**:内置预设现覆盖 页眉页脚(标题居中+页码)/ 水印 / 公式编号 / H1 前分页;切换预设时 toast 列出本次被覆盖的组;「恢复默认」与自定义预设仍只动排版与页面,不污染外壳;切换预设不改变目录模式。
  - **向导补全高频项**:成书向导内可直接设置 边距 / 字体微调 / 标题档位 / 编号开关 / 输出目录 / 导出后行为 / 页眉 logo,与抽屉手动设置等价产出 docx/pdf。
  - **首次启动引导**:首次启动出现引导路径(选预设 → 向导 → 转换),可跳过、不强制打开设置抽屉;尊重系统「减少动效」偏好;非首次启动不再出现。
  - **实测修复(用户反馈)**:向导第五步选了多个文件后「下一步」不再误判为不可用;矮窗口下完成结果区不再出现多余滚动条;主窗口不再因结果区内容变多而上下跳动;向导与弹窗对话框内容不再溢出、文件路径完整可见;toast 提示不再被抽屉或弹窗遮罩遮挡。
  - **内部质量**:修复 CI smoke 菜单守卫陈旧断言(3.6.0「关于」入口移至标题栏信息图标后帮助菜单已移除,守卫仍要求帮助菜单致 smoke 早退、连带中止 renderer 加载);typecheck/lint/build/本地 smoke 全绿。
