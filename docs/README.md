...
- **查看进度/找需求入口** → `ROADMAP.md`（需求唯一入口,候选区）→ `STATUS.md` / `CHANGELOG.md`
- **交付实测** → `ACCEPTANCE.md`
- **UI 设计** → `design/` 目录
...
| `ROADMAP.md` | 需求范围/选型/架构/里程碑（**需求唯一入口**,候选区在「候选区」节） | 规划变更或里程碑完成时 |
...
| `campaigns/<工作项ID>-<说明>/` | 大型重构的阶段细节活载体（本轮 = `campaigns/REF-001-全库优化收尾/`） | 阶段收尾；全部阶段关闭后按归档五步删除 |
