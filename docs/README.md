# docs/ 文档索引

markdown-to-word: Markdown 转 Word / PDF 的 Windows 桌面应用。项目知识库：跨会话需要保留的结论、决策、手册都在本目录；本文件是索引与维护约定。

## 阅读路径（按场景）

- **新手上手** → `USER-GUIDE.md`（使用说明）→ `STATUS.md`（当前状态）
- **搭环境/跑命令/找代码** → `DEV-GUIDE.md`
- **追溯设计决策** → `RESEARCH.md` / `ADR.md`
- **排查故障** → `RESEARCH.md`（坑/根因沉淀）
- **查看进度/找需求入口** → `BACKLOG.md`（项目 backlog,需求唯一入口）→ `ROADMAP.md` / `STATUS.md` / `CHANGELOG.md`
- **交付实测** → `ACCEPTANCE.md`
- **UI 设计** → `design/` 目录

## 文档登记

| 文档 | 用途 | 更新时机 |
|---|---|---|
| `STATUS.md` | 当前状态/验证基线/打开事项 | 每次收尾 |
| `CHANGELOG.md` | 变更历史（面向用户） | 每次发版 |
| `RESEARCH.md` | 库/技术事实、调研结论 | 得出已验证结论时 |
| `ADR.md` | 架构决策（ADR） | 做架构决策/审查时 |
| `ROADMAP.md` | 需求范围/选型/架构/里程碑 | 规划变更或里程碑完成时 |
| `BACKLOG.md` | 项目 backlog（所有需求唯一入口,分类 + 业务价值/工作量评估） | 新需求登记/处置变更时 |
| `DEV-GUIDE.md` | 环境/命令/代码地图/验证方式 | 环境或代码结构变化时 |
| `USER-GUIDE.md` | 终端用户使用说明（安装/操作/设置/FAQ） | 功能或设置变化时 |
| `ACCEPTANCE.md` | 批次验收清单与实测结果记录 | 验收变更/实测完成时 |
| `WPS-COMPAT.md` | Word/WPS 双实测兼容矩阵 | 实测完成/发现问题时 |
| `design/ui-guidelines.md` | UI 视觉规范（冷灰纸+朱砂红、字体三角色、签名元素） | UI 设计变更时 |
| `design/settings-ia.md` | 设置信息架构（设置面板布局与交互） | 设置重组时 |
| `design/book-wizard.md` | 成书向导设计（UI/交互稿） | 向导功能变更时 |
| `design/ui-mockup.html` | UI 交互原型（HTML） | 界面重构时 |
| `OPTIMIZATION-PLAN.md` | 全库代码优化开发计划（裁决、阶段、验收与回退） | 优化排期/裁决变更时 |

## 调研存档清单

> 已归档至 `docs/archive/`，不参与 README 索引跟踪，仅供自查。

| 存档(archive/) | 主题 |
|---|---|
| `2026-08-24-134811-全库审计总报告.md` | 全库审计（五路子审计+总报告+61 项待办清单） |
| `2026-08-24-193838-审计整改裁决与不做项.md` | 审计整改裁决记录 |
| `2026-08-02-2131-竞品功能矩阵与用户需求调研.md` | 竞品功能矩阵、用户需求 |
| `2026-08-05-2151-pdf书签注入调研.md` | PDF 书签注入方案 |
| `2026-08-05-2210-页眉页脚页码调研.md` | 页眉页脚页码方案 |
| `2026-08-06-2116-docx标题编号与内部链接调研.md` | docx 标题编号与内部链接 |
| `2026-08-06-2229-批次6公式链路调研.md` | 批次 6 公式链路（KaTeX/OMML） |
| `2026-08-08-1142-docx域API调研.md` | docx 域 API（TOC/SEQ/REF） |
| `20260813-193532-mermaid集成方案.md` | Mermaid 渲染导出方案 |
| `20260816-114520-文档加密调研.md` | 文档加密调研 |
| `2026-08-23-133005-全库质量审计.md` | 1.0.0 后全库审计 |
| `20260823-230554-目录结构优化方案.md` | 目录/文件结构重组方案 |
| `2026-08-25-182036-功能候选调研与迭代排期.md` | 2.0.0 后功能开发调研 |
| `20260925-004712-代码分析四路盘点.md` | 四路代码分析(core/GUI/测试/文档;处置计划见下行存档) |
| `20260925-130122-技术债处置计划.md` | 技术债处置计划(A-E 五批 24 项全部完成;四路盘点的处置计划) |
| `2026-09-25-174334-竞品功能矩阵调研.md` | 竞品功能矩阵二轮调研（活跃开发三方向输入） |
| `20260925-194556-转换核心深审.md` | 转换核心只读深审（资源预算、取消、双管线与解析边界） |
| `20260925-194556-测试工程化评审.md` | 测试与工程化只读评审（门禁、覆盖、runner、构建与 CI） |
| `20260925-194556-主进程安全评审.md` | Electron 主进程与打包安全只读评审（IPC、持久化、文件与供应链） |
| `20260925-194556-依赖发布风险评审.md` | 依赖与发布风险只读评审（Node、签名、构建可追溯、体积与安全扫描） |
| `20260925-194556-界面体验视觉评审.md` | renderer UI/UX 只读评审（交互、无障碍、国际化与视觉契约） |
| `20260925-194556-整体架构评审.md` | 整体架构与工程质量只读评审（状态契约、边界、生命周期与优先级） |
| `20260925-201811-全库优化评审整合与迭代计划.md` | 六份评审整合、优先级问题登记、决策清单与分阶段代码优化计划（含六份原文完整附录） |

## 维护约定

- 条目格式遵循全局 AGENTS.md「子代理结论落盘」节（结论/理由/来源/关联）。
- 累积型文件（研究结论/架构决策/CHANGELOG）：行数自然增长，不设上限、不做全量审计；新条目在上，同主题更新旧条目并置顶。
- 使用中顺手清理：读到已失效的研究结论 → 删除该条目；读到被推翻的架构决策 → 旧条目首行标「已被 YYYY-MM-DD HH:mm:ss 条目取代」，保留不删。
- 状态速查是每会话入口，保持 ≤80 行：收尾更新时顺手删除已关闭的打开事项与过期状态，超线当场瘦身。
