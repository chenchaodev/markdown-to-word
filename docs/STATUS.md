- 2026-08-24:**发版 1.2.0 完成**(审计改进第三批+目录结构重组:B9 UX 体验批/B13 暗色模式/B12 IPC 面整理/目录重组 6 批;GUI 实测通过;版本号三统一 package.json=tag v1.2.0=CHANGELOG [1.2.0];Release run success 2m37s,资产 MarkdownToWord-Setup-1.2.0.exe + latest.yml;测试 44→45 段;审计排期 B1-B14 与目录结构重组全部关闭,无遗留待办)
- 2026-08-24:**GUI 实测全部通过+目录重组批⑥补遗完成**
- 2026-08-23:**B12/B13/目录结构重组全批完成,待 GUI 实测**(B13 暗色模式 5a91a4a:CSS 变量双主题+设置三态;B12 IPC 面整理 2df5e35:channel 域:动作单源+progress 带 mode;目录重组 5 批 6f3d72a/b1e50e9/061e8dd/d31cb21/2819a2a:i18n 拆分+core 归组/docx handlers/renderer 六域+events 拆 4+1+style 拆四文件/main index 抽取/converter 拆分+smoke 迁出生产路径+resource-dirs 合并;前置再探查 exp-1 核对欠账成立性);每批独立提交 typecheck/lint/build/45 段/smoke 全绿;待实测清单见 ACCEPTANCE.md(B9/B13/重组回归三节),实测通过后发版 1.2.0
- 2026-08-23:**发版 1.1.0 完成**(审计改进第二批:B6 i18n 收口/B4 失败可见性/B5 性能/B7 契约单源解环/B8 大文件拆分/B11 测试盲区补齐+fixtures 存量修复,约 30 项;重构批行为零变化,zh 文案逐字等价;版本号三统一 package.json=tag v1.1.0=CHANGELOG [1.1.0];docx render.ts 1262→467 行/pdf 790→209/renderer 705→147;测试 40→44 段)
- 2026-08-23:**审计改进 B6-B8+B11 全批完成**(10 提交:9d6a2d5/d6dd721/3ebec63/089eac3/e471d2d/0694814/20ed1c8/0a6c9ce/dd9dfbd/083de0d);每批 typecheck/lint/build/test/smoke 全绿后独立提交;check:fixtures 存量漂移已修(code-highlight 段补生成);ROADMAP 审计排期仅余 B9/B13/B12(GUI 功能批)
- 2026-08-23:**发版 1.0.1 完成**(审计改进第一批:B1 安全加固/B2 健壮性/B3 数据渲染正确性/B10 工程基建/B14 文档修正,约 40 项;版本号三统一 package.json=tag v1.0.1=CHANGELOG [1.0.1];Release run success,资产 MarkdownToWord-Setup-1.0.1.exe + latest.yml;builder-debug.yml 误传已删并修 workflow glob 收紧 f293483;CI 首跑 success)
- 2026-08-23:**审计改进 B10c 完成,B10 全批关闭**(tsconfig 启用 noUncheckedIndexedAccess;3 子代理按域并行适配约 145 处严格索引错误——pdf 域 113/docx 域 52/main+renderer 域 20,修复全部行为等价:循环边界/正则捕获组/token 契约处 `!`+不可达依据注释为主,无新增运行时分支);typecheck/lint/build + 40 段 + smoke 全绿。B10 九项(ci.yml/release 加固/userData 隔离/看门狗/incremental/noUncheckedIndexedAccess/删死配置/smoke 守卫/copy-renderer 清理/icons/jszip 统一共 11 项)全部完成
