...
- 验收脚本:`scripts/make-batch4-sample.mjs` 10 段断言(编码预检/脚注/页眉页脚/书签/编号/白名单/公式/TOC 与题注/公式编号与交叉引用);main 侧行为(重名序号/输出目录/取消/批量导出一致)走 smoke + GUI 实测清单(脚本无法触达 main IPC 层)
