...
- 2026-08-28:**F8 合并总目录增强完成(待发版)**:现状合并已是单 convert 通路(mergeMarkdowns → convert 一次),标题/题注编号本就跨文件连续、TOC 本就覆盖全文;F8 仅固化「合并总目录覆盖全部源文件标题 + 跨文件页码准确」行为(无需新增核心代码,仅补测试防止回归);自动断言见 test/segments/merge-toc.test.js(docx+pdf 双格式总目录覆盖 A+B 共 8 标题、PDF 跨文件页码单调且 B>A、.toc-page 注入);typecheck/lint/build/61 段/smoke 全绿;GUI 实测待用户复核(ACCEPTANCE F8)
