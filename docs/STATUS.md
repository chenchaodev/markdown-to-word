...
- 2026-09-25:**需求管道重组,ROADMAP-CANDIDATES 升格项目 backlog(所有需求唯一入口)**:候选池全文重写(待拍板/暂缓/已明确不做/维持人工四层,按功能新增/体验/架构/防御分类,逐项业务价值+工作量评估);ROADMAP 删处置小节(砍/记录不排期/暂不执行项/测试遗留/功能候选/候选池晋升/已知限制/维持人工不自动化)并入候选池,已完成历史保留;指针同步(RESEARCH/计划归档注记/runner/README);规则写入项目 AGENTS「流程」与全局 WORKFLOW-GUIDE 阶段 0/1(v1.9)
