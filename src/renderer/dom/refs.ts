 ...
 * 常量名与语义原样保留,renderer.ts 经命名导入使用。
 * 模块加载时机与 renderer.ts 相同(script type=module),元素必然已就绪。
 * 单文件卡退役(统一队列卡),旧单文件态元素移除;新增快速参数条三引用(qb 前缀语义)。
 */
...
// 格式分段控件(顶栏;radio 语义保留,name="format" 与绑定不变)
...
// 快捷键提示随模式切换(单文件/批量语义),span 固定 id 于动作栏内
...
// name 全文档成组——快速参数条的镜像分段(同名 radio)自动并入本组,
// 绑定与回填零改动覆盖两处控件
...
// 正文字号改 stepper 形态(input id 不变,± 步进按钮为新增元素)
...
// 行距改 range 滑杆(input id 不变)+ mono 实时回显
...
// 标题排版粒度:标题字号/间距档位(自 select 改 seg 分段,三档;组绑定模式同上)
...
// 对齐方式由布尔 checkbox 升级为枚举分段(radio 组,left/justify);
...
// 页眉页脚:模式/文字/logo 选择清除与回显/布局/页脚开关
// 页眉模式与页眉布局自 select 改 seg 分段(模式 + 条件字段容器,
...
// PDF 样式 CSS 导入(导入 / 清除 / 状态显示)
// 补 IA 06 规定的 textarea 形态(与导入/清除同写 settings.pdfCss)
...
// docx 模板导入(浅导入 v1)按钮(预设·管理动作行第四个钮)
...
// 统一队列卡头部动作。previewBtn 仅单文件可见(updateActionButtons
...
// 拖放反馈细化:被跳过的非 Markdown 文件名列表(可折叠;复用汇总条 details 样式类)
...
// 设置抽屉:overlay 根容器(开合记忆 ui-state.panelOpen.page)+ 顶栏入口
...
// ── 快速参数条(舞台内底部;预设/纸张/方向/输出目录四项高频直达)──
