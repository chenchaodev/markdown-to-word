/**
 * renderer 事件绑定组合入口(B8 自 renderer.ts 抽出;批③按域拆分):
 * bindEvents() 保留为唯一对外签名(组合根 renderer.ts 零感知),内部逐域调用:
 ...
 * 调用顺序 = 原单函数内的注册顺序(selection → drop → convert-actions →
 * dialogs-events);跨域仅 document keydown 两处(快捷键 / Esc)同元素同类型,
 * 二者按键互斥,顺序无行为影响。时序不变:绑定先于设置回填。
 */
