 ...
 */
import { replaceMarkdownOutsideProtectedRegions } from "./ai-cleanup.js";
...
export function normalizeObsidian(md: string, options: ObsidianOptions = {}): string {
  ...
  return replaceMarkdownOutsideProtectedRegions(md, (source) =>
    source.replace(WIKILINK_RE, (_whole, bang, inner) => {
      let target = inner;
      let alias = "";
