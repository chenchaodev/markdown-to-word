 ...
 * - **已知 key 守卫**:块内未命中任何已知 key(title/author/date)→ 不视为 frontmatter,
...
正文`);
  ...
  // ---- 10. 空 frontmatter 块(已知 key 守卫:块内无已知 key 一律不剥除) ----
  // 正则 ^[ \t]*---[ \t]*\r?\n([\s\S]*?)\r?\n---[ \t]*(?:\r?\n|$):关闭定界后仅剥除
  // 一个换行;定界相邻时内容组后缺 \r?\n → 整体不匹配(整块当 body,不抛错)。
  ...
  // 空行分隔(---\n\n---)→ 已知 key 守卫:无已知 key 不视为 frontmatter,body 原样保留
  ...
  // ---- 16. 已知 key 守卫核心场景:以 --- 分隔线开头的普通文档,夹层文字不得丢失 ----
  const prose = "---\n这是一段被分隔线包裹的普通文字。\n---\n\n# 正文标题\n";
