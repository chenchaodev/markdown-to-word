// @ts-check
/**
 * 预设 JSON 导入/导出纯函数直测(src/main/persist/preset-file.ts parsePresetsFile/mergePresets):
 * 纯 Node 段(经 dist/main/persist/preset-file.js 导入,该模块零 electron 依赖;
 * 不触对话框/文件 IO——那部分走 GUI 实测)。
 * 断言面(与实现逐条对应):
...
/**
 * 预设夹具条目:字段可残缺(被测的 sanitize 负责补默认/钳制/丢弃),
 * 故对外按契约形状参与 JSON 化与 merge 入参。pageSetup 允许传字符串——
 * 那是「非法条目应被丢弃」用例故意构造的坏形状。
 * @param {string} name 预设名
 * @param {Partial<TypographySettings>} [typography] 排版字段(默认全缺)
 * @param {Partial<PageSetup> | string} [pageSetup] 页面字段(默认全缺;字符串=坏形状)
 ...
 */
const parse = (text) => /** @type {ParsePresetsResult} */ (parsePresetsFile(text));

/**
 * 合并预设(incoming 覆盖 existing;返回形状按实现签名声明的契约取)。
 * @param {readonly CustomPreset[]} existing 现有预设
 * @param {readonly CustomPreset[]} incoming 导入预设
 * @returns {MergePresetsResult} 合并结果
 */
...
export async function run() {
  ...
  for (const r of [
    parse("[]"),
    parse(JSON.stringify({ schemaVersion: 1, presets: [] })),
    parse(JSON.stringify({ schemaVersion: 1, presets: [{ name: "", pageSetup: {} }] })),
    parse(JSON.stringify({ schemaVersion: 1 })), // 缺 presets 字段
  ]) {
  ...
  const r7 = parse(JSON.stringify({ schemaVersion: 1, presets: many }));
  assert(r7.ok && r7.presets.length === 10 && r7.presets[0]?.name === "p0", "超 10 条应截断(保留先出现)");
  console.log("[ok] presets-import:parse(schemaVersion:1/裸数组/坏 JSON/版本非 1/非法丢弃+钳制/空 presets/截断 10)断言通过");
  ...
  const m2 = merge([preset("A")], [preset("C"), preset("D")]);
  ...
  const m3 = merge(e3, [preset("i0"), preset("i1"), preset("i2")]);
  ...
  const m5 = merge(e5, [preset("A1", { bodySizePt: 16 }), preset("B"), preset("C")]);
  ...
  const [firstE8] = e8;
  const [firstI8] = i8;
  assert(firstE8 !== undefined && firstI8 !== undefined, "入参夹具各应有一条");
  assert(firstE8.typography.bodySizePt === undefined, "existing 入参不应被修改");
  assert(firstI8.typography.bodySizePt === 17, "incoming 入参不应被修改");
  console.log("[ok] presets-import:merge(同名覆盖/纯追加/截断/计数/边界/入参不改)断言通过");
}