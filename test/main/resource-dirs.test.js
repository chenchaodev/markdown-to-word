 ...
 *   编译产物恒在 <项目>/dist/main/,../../node_modules 三场景一致(设计见源码头注)
...
export async function run() {
  ...
  // dev/test:编译产物在 <项目>/dist/main;打包:app.asar/dist/main → app.asar/node_modules
  const distMainDev = path.join(ROOT, "dist", "main");
  assert(
    resolveMermaidDir(distMainDev) === path.join(ROOT, "node_modules", "mermaid", "dist"),
    "dev/test 态(moduleDir=<项目>/dist/main)应解析到 <项目>/node_modules/mermaid/dist",
  );
  const distMainPackaged = path.join(asarRoot, "dist", "main");
  assert(
    resolveMermaidDir(distMainPackaged) === path.join(asarRoot, "node_modules", "mermaid", "dist"),
    "打包态(moduleDir=app.asar/dist/main)应解析到 app.asar/node_modules/mermaid/dist",
  );
  console.log("[ok] resource-dirs:resolveMermaidDir 三态同构(dist/main 相对定位)");

  // ---- 4. getMermaidDir 实际态:本环境应指向真实存在的 mermaid dist(含 IIFE 产物) ----
  const mermaidDir = getMermaidDir();
  assert(
    mermaidDir === resolveMermaidDir(path.join(ROOT, "dist", "main")),
    `getMermaidDir 应等于 resolveMermaidDir(<项目>/dist/main),实际 ${mermaidDir}`,
  );
